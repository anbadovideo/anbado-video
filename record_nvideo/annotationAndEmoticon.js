...
//    $("#textinput1").hover(function(){getFocus();});
    ...
    CLIENTVAR.popcornobj= Popcorn.youtube( "#youtube", "http://www.youtube.com/watch?v=t58WCSn5lNI" );
    ...
    CLIENTVAR.popcornobj.on("playing", function() {
        ...
        CLIENTVAR.popcornobj.on("timeupdate", function(){
            ...
            var name=CLIENTVAR.arrayg[parseInt(CLIENTVAR.popcornobj.currentTime())][1];
        //console.log("time:"+name);
        var syntex=$("<div id='timtag'></div>").text("hello"+(name-0.5));//시간에따라 업데이트 되는 툴팁 html생성


       $("#timtag").remove();
              $("tim").append(syntex);
              $("#timtag").css({"top":780 ,"left":500 });


            sinkRecord();
            tooltip();// 툴팁 관련 nvd3.js에서 함수 불러오는 부분
